...
def render_text(position, text_string, font_size=18, color=(255, 255, 255)):
    ...
    glTexCoord2f(1, 0);
    glVertex2f(position[0] + text_size[0] / 8, position[1])
    glTexCoord2f(1, 1);
    glVertex2f(position[0] + text_size[0] / 8, position[1] + text_size[1] / 8)
    glTexCoord2f(0, 1);
    glVertex2f(position[0], position[1] + text_size[1] / 8)
    glEnd()
...
def display():
    global i, scf, sci, level, lflag, wflag, b1x, b1y, bspd, hm, scs, slevel
    ...
    if ((i == 730 or i == -700)  # top and bottom checking
        ...
        render_text((17, 24), "CLICK AND HOLD LEFT MOUSE BUTTON TO GO UP RELEASE TO GO DOWN", 15)
        draw_copter()
        glfw.swap_buffers(window)
    else:
        ...
        scf += 0.025  # so less as program runs very fast
        sci = int(scf)
        scs = str(sci)
        ...
        draw_copter()
        # Code for helicopter
        # If wall moves towards left & gets out of projection volume
        if b1x < -10:
            b1x = 50  # total width is 50
            b1y = random.randint(20, 45)
        else:
            b1x -= bspd
        # Within the projection volume decrease its x value by block_speed
